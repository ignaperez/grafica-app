...
@section('topbar-actions')
    <a href="{{ route('clientes.index') }}" class="gbtn gbtn-ghost gbtn-sm">← Volver</a>
@endsection
...
<div style="max-width:680px;">
    ...
    <form action="{{ route('clientes.store') }}" method="POST" id="form-cliente">
            ...
            <div class="gcard-bd" style="display:flex;flex-direction:column;gap:20px;">
                ...
                <div class="gfg">
                    ...
                    <input type="text" name="nombre" id="nombre" class="ginput"
                           value="{{ old('nombre') }}" required autocomplete="off">
                ...
                <div class="gfg">
                    ...
                    <div style="display:flex;gap:10px;align-items:flex-start;">
                        ...
                        <button type="button" id="btn-consultar" class="gbtn gbtn-ghost gbtn-sm"
                                style="white-space:nowrap;margin-top:1px;" disabled>
                            Consultar ARCA ↗
                        </button>
                    </div>
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:4px;">
                        ...
                        <label style="display:flex;align-items:center;gap:6px;cursor:pointer;
                            ...
                            <input type="radio" name="condicion_iva" value="{{ $val }}"
                                   {{ old('condicion_iva', 'consumidor_final') === $val ? 'checked' : '' }}
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    <div class="gfg">
                        ...
                        <input type="text" name="telefono" class="ginput" value="{{ old('telefono') }}">
                        @error('telefono')<div class="gerr">{{ $message }}</div>@enderror
                    </div>
                    <div class="gfg">
                        <label class="glabel">Email</label>
                        <input type="email" name="email" class="ginput" value="{{ old('email') }}">
                ...
                <div class="gfg">
                    ...
                    <textarea name="direccion" id="direccion" class="gtextarea" rows="2">{{ old('direccion') }}</textarea>
                    ...
                    <select name="lista_precio_id" id="lista_precio_id" class="gselect" required>
                        ...
                        @foreach($listas as $lista)
                            <option value="{{ $lista->id }}"
                                {{ old('lista_precio_id', $lista->nombre === 'Consumidor Final' ? $lista->id : '') == $lista->id ? 'selected' : '' }}>
                                {{ $lista->nombre }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div style="display:flex;gap:10px;margin-top:20px;">
            <button type="submit" class="gbtn gbtn-primary">Guardar cliente</button>
            <a href="{{ route('clientes.index') }}" class="gbtn gbtn-ghost">Cancelar</a>
        </div>
    ...
    $('#btn-consultar').on('click', function () {
        ...
        $.get('{{ route('clientes.consultar-cuit') }}', { cuit })
            .done(function (data) {
                ...
                $status.html('<span class="ok">✓ Datos obtenidos de ARCA — ' + estadoBadge + ivaNote + '</span>');
            })
            .fail(function (xhr) {
